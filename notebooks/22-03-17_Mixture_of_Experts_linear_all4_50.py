...
#%% Pretrained y_plus 50 and pr0.025

model_name_pr0025 = 'zany-snowball-67'
...
pretrained_model_pr0025 = Model(load_pretrained_model_pr0025.input,load_pretrained_model_pr0025.layers[-1].output)
...
#%% Pretrained y_plus 50 and pr0.2

model_name_pr02 = 'crisp-galaxy-64'
...
pretrained_model_pr02 = Model(load_pretrained_model_pr02.input,load_pretrained_model_pr02.layers[-1].output)
...
#%% Pretrained y_plus 50 and pr0.71

model_name_pr071 = 'skilled-planet-65'
...
pretrained_model_pr071 = Model(load_pretrained_model_pr071.input,load_pretrained_model_pr071.layers[-1].output)
...
#%% Pretrained y_plus 50 and pr1

model_name_pr1 = 'stellar-bee-88'
...
pretrained_model_pr1 = Model(load_pretrained_model_pr1.input,load_pretrained_model_pr1.layers[-1].output)
...
cnn9=tf.keras.layers.Conv2DTranspose(4,1)(conc5)
crop=keras.layers.Cropping2D(cropping=padding_layers)(cnn9)

softmax_pixelwise = keras.layers.Softmax(axis=3)(crop)

G0 = keras.layers.Reshape((256,256,1))(softmax_pixelwise[:,:,:,0])
G1 = keras.layers.Reshape((256,256,1))(softmax_pixelwise[:,:,:,1])
G2 = keras.layers.Reshape((256,256,1))(softmax_pixelwise[:,:,:,2])
G3 = keras.layers.Reshape((256,256,1))(softmax_pixelwise[:,:,:,3])

hadamard0 = keras.layers.Multiply()([FCN_pr0025_output, G0])
hadamard1 = keras.layers.Multiply()([FCN_pr02_output, G1])
hadamard2 = keras.layers.Multiply()([FCN_pr071_output, G2])
hadamard3 = keras.layers.Multiply()([FCN_pr1_output, G3])

output = keras.layers.Add()([hadamard0, hadamard1,hadamard2,hadamard3])
...
wandb.init(project="Thesis",notes="Mixture of Experts Linear AllPr y+=50")
