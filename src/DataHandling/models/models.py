...
def final_skip_no_sep_zero_padding(input_features,activation='elu'):
    ...
    for features in input_features:
        ...
        reshape_list.append(reshape)
    conc=keras.layers.Concatenate()(reshape_list)
    conc=keras.layers.ZeroPadding2D(padding=((5,5),(5,5)))(conc)
    ...
    conc5=keras.layers.Concatenate()([conc,batch9])
    output=tf.keras.layers.Conv2DTranspose(1,1)(conc5)
    output=keras.layers.Cropping2D(cropping=5)(output)

    model = keras.Model(inputs=input_list, outputs=output)
    return model


def final_skip_no_sep_padding_test(input_features,activation='elu',padding_layers=1):
    import tensorflow as tf
    import keras
    ...
    conc5=keras.layers.Concatenate()([conc,batch9])
    output=tf.keras.layers.Conv2DTranspose(2,1)(conc5)
    output=keras.layers.Cropping2D(cropping=padding_layers)(output)
    output=keras.layers.Softmax(axis=2)(output)
    x0 = keras.layers.Lambda(lambda x: x[:,:,:,0],name='softmaxGate1')(output)
    x1 = keras.layers.Lambda(lambda x: x[:,:,:,1],name='softmaxGate2')(output)

    model = keras.Model(inputs=input_list, outputs=[x0,x1])
    return model
